...
# 加载图像并进行预处理
image=Image.open("/home/zmw/sam2/image3/original.jpg")
...
plt.axis('on')
# 显示图片
plt.xticks(np.arange(0, image.shape[1], 100))  # 设置x轴刻度，每100个像素一个刻度
plt.yticks(np.arange(0, image.shape[0], 100))  # 设置y轴刻度，每100个像素一个刻度
plt.grid(True)
plt.show()
print('原始图像已显示')
plt.savefig('/home/zmw/sam2/image3/original_original1.jpg')
...
input_point = np.array([[0,1500],[700,1350],[1500,500],[1900,1300],[2000,600],[2200,300]])
...
# 设置横坐标和纵坐标的间隔
x_ticks = range(0, image.shape[1] + 1, 500)  # 横坐标从 0 开始，每隔 50
y_ticks = range(0, image.shape[0] + 1, 500)   # 纵坐标从 0 开始，每隔 25

plt.xticks(ticks=x_ticks)  # 设置横坐标间隔
plt.yticks(ticks=y_ticks)  # 设置纵坐标间隔
# 显示点标记的图片
plt.grid(True)
plt.show()
print('标记图像已显示')
plt.savefig('/home/zmw/sam2/image3/original_output.jpg')
...
# 显示分割结果
save_dir = '/home/zmw/sam2/image3/masks'
