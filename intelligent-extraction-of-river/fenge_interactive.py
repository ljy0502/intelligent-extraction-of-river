...
# 读取带有地理坐标系的 GeoTIFF 图像
def read_geotiff(file_path):
    with rasterio.open(file_path) as src:
        image = src.read()
        image = np.moveaxis(image, 0, -1)  # 调整通道顺序为 HWC
        print("图像数据的类型:", image.dtype)  # 打印图像数据的类型
        return image, src.profile  # 返回像素数据和地理信息
...
image_path = "/home/zmw/sam2/image3/555.tif"
image, profile = read_geotiff(image_path)
image_uint8 = normalize_uint16_to_uint8(image)
print('图像的通道数',image_uint8.shape)
...
for i, mask in enumerate(masks):
    binary_mask = (mask >= 0.5).astype(np.uint8) * 255  # 转换为二值图，阈值为0.5
    binary_mask_file_path = os.path.join(save_dir, f'binary_mask_{i+1}.tif')
    ...
    with rasterio.open(binary_mask_file_path, 'w', **profile) as dst:
        dst.write(binary_mask, 1)
    print(f"Saved mask to {binary_mask_file_path}")
